...
def render_board(n, queens, row=None, safe=None, dead=False, try_col=None, try_safe=None):
    ...
    html = (
        ...
        f"grid-template-columns:repeat({n},{cell_size}px);"
        f"grid-template-rows:repeat({n},{cell_size}px);"
        f"gap:2px;width:{grid_width}px;margin:10px 0;'>"
    )
    safe_set = set(safe) if safe else set()
...
def render_bars(step_data):
    ...
    html = (
        "<div style='display:flex;align-items:flex-end;gap:3px;height:380px;"
        "background:linear-gradient(to top,rgba(15,118,110,0.08),rgba(15,118,110,0.02));"
        "padding:12px;border-radius:12px;border:1px solid #d1d5db;margin-top:10px;'>"
    )
    ...
    for i, val in enumerate(arr):
        ...
        num_div = (
            f"<div style='font-size:11px;margin-top:4px;color:#374151;font-weight:bold;'>{val}</div>"
            if show_numbers else ""
        )
        html += (
            f"<div style='flex:1;display:flex;flex-direction:column;"
            f"align-items:center;justify-content:flex-end;height:100%;'>"
            f"<div style='width:100%;height:{h_pct}%;background:{color};"
            f"border-radius:5px 5px 0 0;transform:scaleY({scale});"
            f"transform-origin:bottom;transition:all 0.1s ease;'></div>"
            f"{num_div}</div>"
        )
...
def safe_eval(expr, scope):
    return np.asarray(
        eval(expr, {"__builtins__": {}}, {**FUNCS, **scope}),
        dtype=float
    )
...
def run_2d(eq, a, b, ylo, yhi, n):
    ...
    for i, yl in enumerate(y_lines):
        ax.plot(x_line, yl, color="#4C9BE8", lw=2,
    ...
    ax.scatter(xr[inside], yr[inside], s=5, alpha=0.5,
               color="#4C9BE8", label=f"Inside ({inside.sum():,})")
    ax.scatter(xr[~inside], yr[~inside], s=5, alpha=0.35,
               color="#E85C5C", label=f"Outside ({(~inside).sum():,})")
    ax.set_xlim(a, b)
    ax.set_ylim(ylo, yhi)
    ax.set_xlabel("x")
    ax.set_ylabel("y")
...
def get_3d_functions(eq):
    ...
    def implicit(x, y, z):
        return (
            safe_eval(lhs, {"x": x, "y": y, "z": z})
            - safe_eval(rhs, {"x": x, "y": y, "z": z})
        )
    ...
    if lhs == "z":
        ...
        def surface(x, y):
            return np.sqrt(np.maximum(
                safe_eval(rhs, {"x": x, "y": y, "z": np.zeros_like(x)}), 0))
    elif rhs == "z**2":
        def surface(x, y):
            return np.sqrt(np.maximum(
                safe_eval(lhs, {"x": x, "y": y, "z": np.zeros_like(x)}), 0))
...
def run_3d(eq, xlo, xhi, ylo, yhi, zlo, zhi, n):
    ...
    xd = np.linspace(xlo, xhi, 60)
    yd = np.linspace(ylo, yhi, 60)
    xx, yy = np.meshgrid(xd, yd)
    zz = surface(xx, yy) if surface is not None else np.full_like(xx, np.nan)
    ...
    def cap(arr_in, mask, limit=8000):
        s = arr_in[mask]
        if len(s) > limit:
    ...
    ax.scatter(xi, yi, zi, s=5, alpha=0.5,
               color="#4C9BE8", label=f"Inside ({inside.sum():,})")
    ax.scatter(xo, yo, zo, s=5, alpha=0.3,
               color="#E85C5C", label=f"Outside ({(~inside).sum():,})")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_zlabel("z")
...
# ===============================================================================
# ── APP 2 & 3 LOGIC: Las Vegas & Backtracking ──────────────────────────────────
# ===============================================================================
...
def generate_lv_steps(n, max_restarts, seed):
    ...
    for attempt in range(1, max_restarts + 1):
        ...
        for row in range(n):
            safe_cols = [
                c for c in range(n)
                if is_safe_queen(row, c, used_cols, used_diag1, used_diag2)
            ]
            steps.append({
                "type": "row_scan", "attempt": attempt,
                "row": row, "safe_cols": list(safe_cols), "queens": list(queens)
            })

            if not safe_cols:
                steps.append({
                    "type": "dead_end", "attempt": attempt,
                    "row": row, "queens": list(queens)
                })
                failed = True
                break
            ...
            steps.append({
                "type": "place", "attempt": attempt,
                "row": row, "col": col, "queens": list(queens)
            })
...
def generate_bt_steps(n):
    ...
    def push(evt_type, row=None, col=None, safe=None):
        out.append({
            "type": evt_type, "queens": list(queens),
            "row": row, "col": col, "safe_col": safe
        })
    ...
    def solve(row):
        ...
        for col in range(n):
            ...
            popped = queens.pop()
            used_cols.remove(popped)
            used_diag1.remove(row - popped)
            used_diag2.remove(row + popped)
...
def generate_qsort_steps(arr, seed):
    ...
    def push(etype, low, high, pivot_idx=None, pval=None,
             cmp_idx=None, swp_i=None, swp_j=None, msg=""):
        c_idx = [cmp_idx] if cmp_idx is not None else []
        s_idx = [swp_i, swp_j] if (swp_i is not None and swp_j is not None) else []
        out.append({
            "type": etype, "array": list(a), "sorted": set(sorted_set),
            "low": low, "high": high, "pivotIndex": pivot_idx,
            "pivotValue": pval, "compareIndices": c_idx,
            "swapIndices": s_idx, "msg": msg
        })

    def partition(low, high):
        p_idx = random.randint(low, high)
        push("pivot-select", low, high, pivot_idx=p_idx,
             pval=a[p_idx], msg=f"Random pivot: idx {p_idx}, val {a[p_idx]}")

        if p_idx != high:
            a[p_idx], a[high] = a[high], a[p_idx]
            push("swap", low, high, swp_i=p_idx, swp_j=high,
                 pivot_idx=high, msg="Move pivot to end")

        pivot = a[high]
        i = low - 1

        for j in range(low, high):
            push("compare", low, high, pivot_idx=high, pval=pivot,
                 cmp_idx=j, msg=f"Compare idx {j} with pivot {pivot}")
            if a[j] <= pivot:
                i += 1
                if i != j:
                    a[i], a[j] = a[j], a[i]
                    push("swap", low, high, swp_i=i, swp_j=j,
                         pivot_idx=high, msg="Partition swap")

        final_p = i + 1
        if final_p != high:
            a[final_p], a[high] = a[high], a[final_p]
            push("swap", low, high, swp_i=final_p, swp_j=high,
                 pivot_idx=final_p, msg="Place pivot")

        sorted_set.add(final_p)
        push("pivot-fixed", low, high, pivot_idx=final_p,
             pval=a[final_p], msg=f"Pivot fixed at idx {final_p}")
        return final_p
    ...
    if run_btn:
        ...
        for step in all_steps:
            ...
            if st_type == "attempt_start":
                msg = f"Attempt {step['attempt']}: Starting new..."
            elif st_type == "row_scan":
                msg = f"Attempt {step['attempt']} | Row {step['row'] + 1}: Scanning safe."
            elif st_type == "place":
                msg = f"Attempt {step['attempt']} | Placed Q at ({step['row'] + 1}, {step['col'] + 1})"
            elif st_type == "dead_end":
                msg = f"❌ Dead-end at Row {step['row'] + 1}!"
            elif st_type == "restart":
                msg = "Restarting..."
            elif st_type == "success":
                msg = f"✅ SUCCESS on attempt {step['attempt']}."
            elif st_type == "failed":
                msg = f"Failed after {step['attempt']} restarts."
            else:
                msg = f"Event: {st_type}"
...
# ── Route 3: 8-Queens Compare (LV vs BT) ───────────────────────────────────────
    ...
    if run_btn:
        ...
        for idx in range(total_steps):
            ...
            lv_board.markdown(
                render_board(
                    n, lv_evt.get("queens", []),
                    row=lv_evt.get("row"),
                    safe=lv_evt.get("safe_cols"),
                    dead=(lv_evt["type"] == "dead_end")
                ),
                unsafe_allow_html=True
            )
            lv_metrics.markdown(
                f"**Step:** {min(idx + 1, len(lv_steps))}/{len(lv_steps)} | **Restarts:** {lv_restarts}"
            )

            bt_evt = bt_steps[idx] if idx < len(bt_steps) else bt_steps[-1]
            if bt_evt["type"] == "row_start":
                bt_calls += 1
            if bt_evt["type"] == "backtrack":
                bt_backs += 1
            bt_msg = f"{bt_evt['type']} "
            if bt_evt.get("row") is not None:
                bt_msg += f"R:{bt_evt['row'] + 1}"
            if bt_evt.get("col") is not None:
                bt_msg += f" C:{bt_evt['col'] + 1}"
            ...
            bt_metrics.markdown(
                f"**Step:** {min(idx + 1, len(bt_steps))}/{len(bt_steps)} "
                f"| **Calls:** {bt_calls} | **Backtracks:** {bt_backs}"
            )
        ...
        st.success(
            ...
            f"Las Vegas took {len(lv_steps)} steps. "
            f"Backtracking took {len(bt_steps)} steps."
        )
    else:
        empty_board = render_board(n, [])
        with col1:
            lv_board.markdown(empty_board, unsafe_allow_html=True)
        with col2:
            bt_board.markdown(empty_board, unsafe_allow_html=True)
...
# ── Route 4: Randomized Quick Sort ─────────────────────────────────────────────
    ...
    if run_btn:
        ...
        for idx, step in enumerate(all_steps):
            ...
            bar_box.markdown(render_bars(step), unsafe_allow_html=True)
            time.sleep(delay)
